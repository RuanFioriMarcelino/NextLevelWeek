...
export default function Ticket(){
    ...
    async function handSelectImage() {
        try {
            ...
            if(result.assets){
                badgeStore.updataAvatar(result.assets[0].uri)
            }
        }catch (error){
            console.log(error)
    ...
    return (
        <View className="flex-1 bg-green-500">
            ...
            <Credential data={badgeStore.data} onChangeAvatar={handSelectImage} onExpandQRCode={() => setExpandQRCode(true)}/>
            <MotiView
                from={{
                    translateY: 0
                }}
                ...
                <FontAwesome name="angle-double-down"color={colors.gray[300]} size={24}
            ...
            <Button title="Compartilhar" onPress={handShare}/>
            <TouchableOpacity activeOpacity={0.7} onPress={() => badgeStore.remove()}><Text className="text-base text-white font-bold text-center mt-10">Remover ingresso</Text></TouchableOpacity>
            </ScrollView>
